...
-- v0.3: 게임 필 패치 — 속도 체감 7.5배, 뽁 연타감, 물방울 팝 사운드,
--       월드 "+N" 플로터, 눌림(squash) 애니메이션, FOV 속도감, 가드레일
...
local CONFIG = {
	BaseWalkSpeed = 16,
	SpeedToWalk = 0.45,      -- Speed 1당 걷기속도 보너스 — "빨라지는 게 몸으로 느껴져야" 한다
	MaxWalkSpeed = 130,
	TileCooldown = 0.35,     -- 매 걸음이 뽁이어야 손맛이 난다
	PopRespawn = 1.2,        -- 터진 뽁뽁이 복구 시간
	TileSize = 5,
...
local function popSound(parent)
	...
	-- 내장 사운드 중 '뽁'에 가장 가까운 물방울 퐁 소리. 높은 랜덤 피치 = 작고 귀여운 팝
	-- 더 좋은 ASMR 팝을 원하면: Studio 툴박스에서 "bubble pop" 검색 → 아래 SoundId만 교체
	sound.SoundId = "rbxasset://sounds/impact_water.mp3"
	sound.PlaybackSpeed = 1.2 + math.random() * 0.5
...
-- 뽁 터질 때 그 자리에서 떠오르는 "+N" (이 장르의 시그니처 비주얼)
local function popFloater(bubble, amount)
...
local function spawnBubble(x, z, zoneColor)
	-- 눌리기 전(볼록)/눌린 후(납작) 상태를 미리 정의 — 키가 눌리는 촉감의 핵심
	local upCF = CFrame.new(x, 0.55, z) * CFrame.Angles(0, 0, math.rad(90))
	local downCF = CFrame.new(x, 0.18, z) * CFrame.Angles(0, 0, math.rad(90))
	local upSize = Vector3.new(1.1, TILE - 0.9, TILE - 0.9)
	local downSize = Vector3.new(0.3, TILE - 0.4, TILE - 0.4) -- 아래로 꺼지며 옆으로 퍼짐(squash)
	...
	bubble.Touched:Connect(function(hit)
		...
		popped = true
		popSound(bubble)
		popFloater(bubble, gain)
		TweenService:Create(bubble, TweenInfo.new(0.07, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = downSize,
			CFrame = downCF,
			Transparency = 0.6,
			Color = zoneColor,
...
-- 고속 주행 이탈 방지 가드레일 (양옆 반투명 벽)
local mapLength = (z + 24) - (-30)
for _, side in { -1, 1 } do
	local wall = makePart({
		Name = "GuardRail",
		Transparency = 0.92,
		Color = Color3.fromRGB(200, 230, 255),
		Size = Vector3.new(1, 16, mapLength),
		CFrame = CFrame.new(side * ((fieldWidthStuds + 12) / 2 + 0.5), 7.5, (z - 6) / 2),
	})
	wall.Parent = map
end
